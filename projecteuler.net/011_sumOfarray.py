...
matrix = [row1, row2, row3, row4, row5, row6, row7, row8, row9, row10, row11, row12, row13, row14, row15, row16, row17,
          row18, row19, row20]
Tmatrix = [list(i) for i in zip(*matrix)]

cluster = [matrix, Tmatrix]
max = 0
# poziomo i pionowo
for z in cluster:
    for item in z:
...
for i in range(0, 16):
    for j in range(0, 16):
        k = matrix[i+3][j] * matrix[i + 2][j + 1] * matrix[i + 1][j + 2] * matrix[i][j + 3]
        if k > max:
            max = k
print(max)
